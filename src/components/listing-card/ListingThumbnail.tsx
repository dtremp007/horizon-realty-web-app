...
export default function ListingThumbnail({imageUrl, backupUrl, price, currency, status, availability: availibility}: Props) {
    const [src, setSrc] = useState(process.env.NODE_ENV === "development" ? houseThumbnail : imageUrl)
    const toggleSold = getToggleFunction("listing-card--sold", availibility === "sold" ? true : false)

  return (
    <div className="listing-card__thumbnail">
        <Image src={src} onError={() => setSrc(backupUrl)} layout="fill" objectFit="cover" alt="thumbnail"/>
